...
| Panel de solicitudes | `http://localhost:3000/admin?token=…` (el token está en `server/config.json`) |
...
compartir-con-amigos.cmd Arranca la web y la publica con un enlace temporal
...
server/
  ...
  backups/              Copias fechadas de config.json antes de cada cambio
assets/
  css/
...
## Compartir con otra persona (sin publicarla de verdad)

Para que alguien vea tu web tal cual la tienes en local, sin desplegarla en
ningún sitio: doble clic en **`compartir-con-amigos.cmd`**.

Arranca el servidor y abre un **Cloudflare Tunnel**: una URL pública temporal
(`https://algo-al-azar.trycloudflare.com`) que reenvía a tu `localhost:3000`.
No hace falta abrir puertos en el router ni tener IP fija. Compártela con
quien quieras — deja de funcionar en cuanto cierres esa ventana, y cada vez
que arrancas el script te da una URL distinta.

La primera vez necesita [cloudflared](https://github.com/cloudflare/cloudflared/releases/latest)
instalado (`winget install --id Cloudflare.cloudflared -e`, o el `.exe` suelto
en esta misma carpeta); el propio script te avisa si falta.

Como esa URL es pública mientras el túnel está abierto, cualquiera que la
tenga puede llegar también a `/admin` si conoce el token — de fábrica viene
ya con uno aleatorio en `server/config.json` (no es `aithera`), pero cámbialo
por el tuyo si vas a compartir el enlace más de una vez.
